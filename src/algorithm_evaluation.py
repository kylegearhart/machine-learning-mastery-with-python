...
def evaluate_regression_algorithm_using_training_dataset(training_dataset, algorithm, *args):
    ...
    root_mean_squared_error = \
        calculate_root_mean_squared_error(predictions_by_algorithm_on_test_dataset,
                                          correct_predictions_for_test_dataset)
    return root_mean_squared_error


def evaluate_classification_algorithm_with_train_test_split(dataset, algorithm, split_percentage, *args):
...
def evaluate_classification_algorithm_with_k_fold_cross_validation(dataset, algorithm, num_folds, *args):
...
def evaluate_regression_algorithm_with_k_fold_cross_validation(dataset, algorithm, num_folds, *args):
    algorithm_accuracy_on_each_fold = evaluate_algorithm_with_k_fold_cross_validation(
        dataset, algorithm, num_folds, calculate_root_mean_squared_error, *args
    )
    print('Accuracy for algorithm over {0} folds:'.format(num_folds))
    print(' '.join(str('%.7f' % accuracy) for accuracy in algorithm_accuracy_on_each_fold))
    print('Mean Root Mean Squared Error: %.3f\n' % (sum(algorithm_accuracy_on_each_fold) / float(num_folds)))


def evaluate_algorithm_with_k_fold_cross_validation(dataset, algorithm, num_folds, prediction_accuracy_fn, *args):
    ...
    for test_fold in folds:
        ...
        accuracy_of_algorithm_on_current_fold = \
            prediction_accuracy_fn(predictions_by_algorithm_on_test_dataset,
                                   correct_predictions_for_test_dataset)
        algorithm_accuracy_on_each_fold.append(accuracy_of_algorithm_on_current_fold)
    return algorithm_accuracy_on_each_fold
